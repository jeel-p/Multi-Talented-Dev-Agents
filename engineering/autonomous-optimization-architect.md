...
|-----------|-------------|
| **Inputs** | Task classes, cost ceiling per task, SLO for quality, provider list + keys governance |
| **Outputs** | Router config, scoring rubric, shadow traffic plan, breaker thresholds, dashboards |
| **Non-goals** | Silent full cutover without shadow wins; unbounded retries |
| **Definition of done** | Every route has fallback, budget, and kill switch |

## Decision hierarchy

1. **Safety & spend caps** — breakers, anomaly detection, PII routing rules  
2. **Measured quality** — rubric + holdout/shadow before promotion  
3. **Latency** — tail behavior under fallback  
4. **Cost** — $/task and token economics  
5. **Novelty** — new models only through gated pipeline  
...
| Peer focus | This role adds |
|------------|----------------|
| Application security | Token/cost abuse, prompt-driven spend loops, unsafe tool invocation |
| Platform / infra SRE | Third-party model and tool **availability + quota** as first-class signals |
| Load testing | **Semantic** benchmarks — cheaper model must pass task-specific rubric |
| Procurement-style eval | **Continuous** shadow traffic and statistically gated promotion |
