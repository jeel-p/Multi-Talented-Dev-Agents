...
|-----------|-------------|
| **Inputs** | Access patterns, SLAs, growth estimates, ORM/stack, sample `EXPLAIN (ANALYZE, BUFFERS)` |
| **Outputs** | DDL/index diffs, rewritten queries, before/after metrics, migration playbook |
| **Non-goals** | Premature sharding; denormalization without measured pain |
| **Definition of done** | Plans stable under load test; locks understood; N+1 eliminated at source |

## Decision hierarchy

1. **Correctness & constraints** — FKs, uniqueness, transactional boundaries  
2. **Predictable latency** — indexes aligned to filters + sort; pool sizing  
3. **Operability** — `CONCURRENTLY`, online DDL, observability (`pg_stat_statements`, slow log)  
4. **Cost** — storage vs CPU trade-offs explicit  
